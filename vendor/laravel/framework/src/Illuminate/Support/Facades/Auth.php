...
class Auth extends Facade
{
    ...
    public static function storeImg($request, $namefile)
    {
        $link_img = $request->file($namefile);
        if ($link_img != NULL) {
            $user = Auth::user();
            $file_stogare = ($user->cccd);
            $link_img->storeAs("public/upload/$file_stogare/", "$namefile.jpg");
        }
    }
}
